...
use super::{ArmCond, ArmHalfwordTransferType, ArmInstruction};
use crate::arm::ArmDecodeHelper;
use crate::registers::StatusRegister;
use crate::{
    alu::{AluOpCode, BarrelShiftOpCode, BarrelShifterValue, ShiftRegisterBy, ShiftedRegister},
    reg_string,
};
use crate::{Addr, REG_PC};
use std::fmt;
...
impl ArmInstruction {
    ...
    fn fmt_msr_flags(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        if let Ok(Some(op)) = self.fmt_operand2(f) {
            let psr = StatusRegister::from(op & 0xf000_0000);
            write!(
                f,
                "\t; N={} Z={} C={} V={}",
                psr.n(),
