use crate::{Addr, InstructionDecoder};
use std::fmt;
...
where
    D: InstructionDecoder,
{
    pub fn new(base: Addr, bytes: &'a [u8]) -> Self {
        Self {
            base,
            pos: 0,
            bytes,
    ...
    D::IntType: fmt::LowerHex,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let decoded: D = D::decode_bytes(&self.bytes[(self.pos as usize)..], addr);
