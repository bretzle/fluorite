use std::fmt;

use crate::{cpu::Arm7tdmi, Addr};

#[derive(Debug, Copy, Clone)]
pub enum MemoryAccess {
    NonSeq = 0,
    Seq,
    Debugging,
}

impl Default for MemoryAccess {
    fn default() -> MemoryAccess {
        MemoryAccess::NonSeq
    }
}

impl fmt::Display for MemoryAccess {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "{}",
            match self {
                MemoryAccess::NonSeq => "N",
                MemoryAccess::Seq => "S",
                MemoryAccess::Debugging => todo!(),
            }
        )
    }
}

#[derive(Debug, PartialEq, Copy, Clone)]
#[repr(u8)]
pub enum MemoryAccessWidth {
    MemoryAccess8 = 0,
    MemoryAccess16,
    MemoryAccess32,
}

pub trait MemoryInterface {
    fn load_8(&mut self, addr: Addr, access: MemoryAccess) -> u8;
    fn load_16(&mut self, addr: Addr, access: MemoryAccess) -> u16;
    fn load_32(&mut self, addr: Addr, access: MemoryAccess) -> u32;

    fn store_8(&mut self, addr: Addr, val: u8, access: MemoryAccess);
    fn store_16(&mut self, addr: Addr, val: u16, access: MemoryAccess);
    fn store_32(&mut self, addr: Addr, val: u32, access: MemoryAccess);

    fn idle_cycle(&mut self);
}

#[allow(dead_code)]
impl<Memory: MemoryInterface> Arm7tdmi<Memory> {
    pub(crate) fn load_8(&mut self, addr: u32, access: MemoryAccess) -> u8 {
        self.bus.load_8(addr, access)
    }
    pub(crate) fn load_16(&mut self, addr: u32, access: MemoryAccess) -> u16 {
        self.bus.load_16(addr & !1, access)
    }
    pub(crate) fn load_32(&mut self, addr: u32, access: MemoryAccess) -> u32 {
        self.bus.load_32(addr & !3, access)
    }

    pub(crate) fn store_8(&mut self, addr: u32, value: u8, access: MemoryAccess) {
        self.bus.store_8(addr, value, access);
    }
    pub(crate) fn store_16(&mut self, addr: u32, value: u16, access: MemoryAccess) {
        self.bus.store_16(addr & !1, value, access);
    }
    pub(crate) fn store_32(&mut self, addr: u32, value: u32, access: MemoryAccess) {
        self.bus.store_32(addr & !3, value, access);
    }

    pub(crate) fn store_aligned_32(&mut self, addr: Addr, value: u32, access: MemoryAccess) {
        self.store_32(addr & !0x3, value, access);
    }

    pub(crate) fn store_aligned_16(&mut self, addr: Addr, value: u16, access: MemoryAccess) {
        self.store_16(addr & !0x1, value, access);
    }

    /// Helper function for "ldr" instruction that handles misaligned addresses
    pub(crate) fn ldr_word(&mut self, addr: Addr, access: MemoryAccess) -> u32 {
        if addr & 0x3 != 0 {
            let rotation = (addr & 0x3) << 3;
            let value = self.load_32(addr & !0x3, access);
            let mut carry = self.cspr.c();
            let v = self.ror(value, rotation, &mut carry, false, false);
            self.cspr.set_c(carry);
            v
        } else {
            self.load_32(addr, access)
        }
    }

    /// Helper function for "ldrh" instruction that handles misaligned addresses
    pub(crate) fn ldr_half(&mut self, addr: Addr, access: MemoryAccess) -> u32 {
        if addr & 0x1 != 0 {
            let rotation = (addr & 0x1) << 3;
            let value = self.load_16(addr & !0x1, access);
            let mut carry = self.cspr.c();
            let v = self.ror(value as u32, rotation, &mut carry, false, false);
            self.cspr.set_c(carry);
            v
        } else {
            self.load_16(addr, access) as u32
        }
    }

    /// Helper function for "ldrsh" instruction that handles misaligned addresses
    pub(crate) fn ldr_sign_half(&mut self, addr: Addr, access: MemoryAccess) -> u32 {
        if addr & 0x1 != 0 {
            self.load_8(addr, access) as i8 as i32 as u32
        } else {
            self.load_16(addr, access) as i16 as i32 as u32
        }
    }

    pub(crate) fn idle_cycle(&mut self) {
        self.bus.idle_cycle()
    }
}
