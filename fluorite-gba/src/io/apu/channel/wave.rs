    ...
    pub fn clock(&mut self) {
        if self.timer.clock_with_reload(self.calc_reload()) {
            ...
            if self.wave_ram_i == 32 {
                self.wave_ram_i = 0;
                if self.use_two_banks {
                    self.wave_ram_bank ^= 1
                }
            }
        }
    }
...
impl Channel for Wave {
    fn generate_sample(&self) -> i16 {
        ...
        let sample = if self.wave_ram_i % 2 == 0 {
            byte >> 4
        } else {
            byte & 0xF
        };
        let volume_factor = if self.force_volume {
            6
        } else {
            Wave::VOLUME_FACTORS[self.volume as usize]
        };
