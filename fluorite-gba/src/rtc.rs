...
use crate::cartridge::{GpioDevice, GpioDirection, GpioState};
use bit_reverse::LookupReverse;
use chrono::prelude::*;
use fluorite_common::BitIndex;

fn num2bcd(mut num: u8) -> u8 {
    num = std::cmp::min(num, 99);
...
impl Rtc {
    ...
    fn force_reset(&mut self) {
        ...
        // TODO: the date time registers should be reset to 0-0-0-0..
    }

    fn serial_transfer_in_progress(&self) -> bool {
        match self.state {
            RtcState::Idle | RtcState::WaitForCS => false,
            _ => true,
        }
    }

    fn load_register(&mut self, reg: RegisterKind) {
        match reg {
            ...
            _ => panic!("RTC: read {:?} not implemented", reg),
        }
    }

    fn store_register(&mut self, reg: RegisterKind) {
        match reg {
            RegisterKind::Status => self.status.write(self.internal_buffer[0]),
            RegisterKind::ForceReset => self.force_reset(),
            _ => panic!("RTC: write {:?} not implemented", reg),
        }
    }
}
...
impl GpioDevice for Rtc {
    fn write(&mut self, gpio_state: &GpioState, data: u16) {
        ...
        if self.cs.high() && old_cs.low() {
            println!("RTC: CS went from low to high!");
        }
